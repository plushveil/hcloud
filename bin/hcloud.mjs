...
try {
  ...
  console.log(`> hcloud ${process.argv.slice(2).join(' ')}`)
  console.error(err)
  process.exit(1)
}
...
export default async function main (cwd = process.cwd()) {
  ...
  const hcloud = path.resolve(cwd, ...pkg.main.split('/'))

  return await cli.execute(await cli.parse(hcloud), process.argv.slice(2))
}
